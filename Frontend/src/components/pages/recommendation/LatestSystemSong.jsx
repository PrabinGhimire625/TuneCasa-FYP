...
import { getLatestArtist6Song, getLatestSystem6Song } from "../../../store/songSlice";
import { Link } from "react-router-dom";
import { FaPlay, FaPause } from "react-icons/fa";

const LatestSystemSong = ({ id }) => {
  const dispatch = useDispatch();
  const { latestSystem6Song, status } = useSelector((state) => state.song);
  const { currentSong, isPlaying } = useSelector((state) => state.player);

  useEffect(() => {
    dispatch(getLatestSystem6Song());
  }, [dispatch]);

  useEffect(() => {
    if (latestSystem6Song && latestSystem6Song.length > 0) {
      dispatch(setSongList(latestSystem6Song));
    }
  }, [latestSystem6Song, dispatch]);
    ...
    <div className="text-white px-4 md:px-8 py-10">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-2xl md:text-2xl font-bold">Newly Released Song</h2>

        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
          {latestSystem6Song.slice(0, 6).map((songItem, index) => (
            <div
              key={index}
...
export default LatestSystemSong;
